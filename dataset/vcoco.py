    ...
    def _add_heatmap_to_target(self):
        for i, anno in enumerate(self._anno):
            ...
            for j, keypoint in enumerate(anno["keypoints"]):
                ...
                joints_vis = np.zeros_like(joints)
                selected_index = np.where(joints > 1)
                joints_vis = joints_vis[selected_index]
                heatmaps, weights = self.generate_target(joints, joints_vis)
                self._anno[i]["heatmaps"] = heatmaps
                self._anno[i]["weights"] = weights
