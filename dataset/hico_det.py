...
class HICODetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not osp.exists(img_path):
            log_every_n(40, "Cannot found image data: " + img_path)
        ...
        if num_object == 0:
            ...
            for k in range(num_rels):
                ...
                hoi_labels.append(
                    self.multi_dense_to_one_hot(
                        hoi_label_np,
                        self.num_classes_verb + 1))

                sub_ct_coord = boxes[hoi['subject_id']][..., :2]
                obj_ct_coord = boxes[hoi['object_id']][..., :2]
                hoi_boxes.append(torch.cat(
                    [sub_ct_coord, obj_ct_coord], dim=-1).reshape(-1, 4))
            hoi_labels = np.array(hoi_labels).reshape(-1,
                                                      self.num_classes_verb)

        target['rel_labels'] = torch.from_numpy(hoi_labels)
        if len(hoi_boxes) == 0:
            target['rel_vecs'] = torch.from_numpy(
                np.array([]).reshape(-1, 4)).float()
        else:
