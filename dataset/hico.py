...
hoi_interaction_names = json.loads(
    open(os.path.join(config.DATASET.ROOT, config.DATASET.NAME, config.DATASET.INTERACTION_NAME), 'r').readlines()[0])['verb_names']
...
def get_interaction_box(human_box, object_box, hoi_id):
    ...
    xx1, yy1, xx2, yy2 = min(hx1, ox1), min(
        hy1, oy1), max(hx2, ox2), max(hy2, oy2)
...
def get_hoi_annotation_from_odgt(item, total_boxes, scale):
    ...
    for hoi in item.get('hoi', []):
        ...
        hoi_box = get_interaction_box(
            human_box=human_box, object_box=object_box, hoi_id=hoi_id)
    ...
    return dict(
        ...
        object_boxes=torch.from_numpy(
            np.array(object_boxes).astype(np.float32)),
        object_labels=torch.from_numpy(np.array(object_labels)),
        action_boxes=torch.from_numpy(
            np.array(action_boxes).astype(np.float32)),
...
def parse_one_gt_line(gt_line, scale=1):
    ...
    gt_boxes, ignored_boxes, total_boxes = get_det_annotation_from_odgt(
        item, img_shape, flip=0)
...
class HoiDetection(VisionDataset):
    ...
    def __init__(self, root, annFile, transform=None, target_transform=None, transforms=None, image_set='train'):
        ...
        super(HoiDetection, self).__init__(
            root, transforms, transform, target_transform)
        annotations = [parse_one_gt_line(l.strip())
                       for l in open(annFile, 'r').readlines()]
        if self.image_set in ['train']:
            self.annotations = [a for a in annotations if len(
                a['annotations']['action_labels']) > 0]
        else:
...
def build(image_set, test_scale=-1):
    ...
    if image_set == 'train':
        annotation_file = os.path.join(
            config.DATASET.ROOT, config.DATASET.NAME, config.DATASET.ANNO)
    else:
        annotation_file = './data/hico/hico_test_remake.odgt'

    images_file = os.path.join(config.DATASET.ROOT, config.DATASET.IMAGES)
    dataset = HoiDetection(root=images_file, annFile=annotation_file,
