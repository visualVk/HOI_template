...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes_o = torch.cat([torch.cat([t["labels_o"], t["labels_h"]]).to(
            torch.long).to(t["labels_o"].device)[J] for t, (_, J) in zip(targets, indices)])
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        tgt_lengths = torch.as_tensor(
            [len(v["labels_o"]) + len(v["labels_h"]) for v in targets], device=device)
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_boxes = torch.cat([torch.cat([t["boxes_o"], t["boxes_h"]]).to(
            src_boxes.device)[i] for t, (_, i) in zip(targets, indices)], dim=0)
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        src_masks = F.interpolate(src_masks[:,
                                            None],
        ...
        losses = {
            "loss_mask": sigmoid_focal_loss(
                src_masks, target_masks, num_boxes), "loss_dice": dice_loss(
    ...
    def forward(self, outputs, targets):
        ...
        num_boxes = sum(len(t["boxes_h"]) + len(t["boxes_o"]) for t in targets)
...
def build_detr(config: easydict.EasyDict, args: argparse.Namespace):
    ...
    if config.MODEL.DETR_PRETRAINED:
        model.load_state_dict(torch.load('./data/detr-r50-vcoco.pth', map_location='cpu')['model_state_dict'])

    matcher = build_matcher(config)
    weight_dict = {'loss_ce': 1, 'loss_bbox': config.CRITERION.BBOX_LOSS_COEF}
    weight_dict['loss_giou'] = config.CRITERION.GIOU_LOSS_COEF

    if config.AUX_LOSS:
        aux_weight_dict = {}
        for i in range(config.MODEL.DEC_LAYERS - 1):
    ...
    criterion = SetCriterion(
        ...
        eos_coef=config.CRITERION.EOS_COEF,
        losses=losses)
    criterion.to(device)
