...
class ASNet(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        rel_out = {'pred_logits': outputs_rel_class[-1],
                   'pred_boxes': outputs_rel_coord[-1],
                   'src_emb': src_emb[-1],
                   'dst_emb': dst_emb[-1]}
        output = {
            'pred_det': out,
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_rel_class,
        ...
        for idx in range(len(outputs_class)):
            ...
            if idx < len(outputs_rel_class):
                rel_out = {
                    'pred_logits': outputs_rel_class[idx],
                    'pred_boxes': outputs_rel_coord[idx],
                    'src_emb': src_emb[idx],
                    'dst_emb': dst_emb[idx]}
            else:
                rel_out = None
...
class SetCriterion(nn.Module):
    ...
    def loss_actions(
        ...
        target_classes_obj = torch.cat([t["rel_labels"][J].to(
            src_logits.device) for t, (_, J) in zip(targets, indices)])

        target_classes = torch.zeros(
            src_logits.shape[0],
            src_logits.shape[1],
            self.rel_classes).type_as(src_logits).to(
            src_logits.device)
        ...
        rel_loss = F.binary_cross_entropy_with_logits(
            src_logits, target_classes, reduction='none') * focal_weight
        if loss_reduce == 'mean':
    ...
    def loss_rel_vecs(
        ...
        target_vecs = torch.cat([t['rel_vecs'][i]
                                for t, (_, i) in zip(targets, indices)], dim=0)
    ...
    def loss_emb_push(
        ...
        n = id_emb.shape[0]
        m = [m.reshape(-1)
             for m in torch.meshgrid(torch.arange(n), torch.arange(n))]
    ...
    def loss_emb_pull(
        ...
        for i in range(len(src_emb)):
            ref_idx = torch.where(target_det_centr ==
                                  target_rel_centr[i, :2])[0]
            if len(ref_idx) == 0:
    ...
    def _get_neg_permutation_idx(self, neg_indices):
        ...
        batch_idx = torch.cat([torch.full_like(neg_ind, i)
                              for i, neg_ind in enumerate(neg_indices)])
    ...
    def forward(self, outputs, targets):
        ...
        num_boxes = torch.as_tensor(
            ...
            device=next(
                iter(
                    outputs['pred_det'].values())).device)
        rel_num_boxes = sum(len(t["rel_labels"]) for t in targets)
        rel_num_boxes = torch.as_tensor(
            [rel_num_boxes],
            dtype=torch.float,
            device=next(
                iter(
                    outputs['pred_rel'].values())).device)
        if is_dist_avail_and_initialized():
            torch.distributed.all_reduce(num_boxes)
            torch.distributed.all_reduce(rel_num_boxes)
        num_boxes = torch.clamp(num_boxes / get_world_size(), min=1).item()
        rel_num_boxes = torch.clamp(
            rel_num_boxes / get_world_size(), min=1).item()
        num_boxes_dict = {
            'det': num_boxes,
            'rel': rel_num_boxes
        }
...
class PostProcess(nn.Module):
    ...
    def get_matching_scores(self, s_cetr, o_cetr, s_scores, o_scores, rel_vec,
        ...
        rel_s_centr = rel_vec[...,
                              :2].unsqueeze(-1).repeat(1, 1, s_cetr.shape[0])
        rel_o_centr = rel_vec[...,
                              2:].unsqueeze(-1).repeat(1, 1, o_cetr.shape[0])
        ...
        if self.use_emb is True:
            ...
            dist_s_emb = torch.from_numpy(
                cdist(
                    src_emb_np,
                    s_emb_np,
                    metric='euclidean')).to(
                rel_vec.device)
            dist_o_emb = torch.from_numpy(
                cdist(
                    dst_emb_np,
                    o_emb_np,
                    metric='euclidean')).to(
    ...
    def forward(self, outputs_dict, file_name, target_sizes,
        ...
        dist_s, dist_o = self.get_matching_scores(
            s_cetr, o_cetr, s_scores, o_scores, rel_vec_flat, s_emb, o_emb, src_emb, dst_emb)
        ...
        rel_array = torch.from_numpy(
            np.load(
                self.rel_array_path)).to(
            hoi_scores.device)
        ...
        _, hoi_id = np.unique(
            hoi_triplet[:, [0, 1, 2]], axis=0, return_index=True)
        ...
        rel_s_ids, rel_o_ids = np.array(rel_triplet[..., 0], dtype=np.int64), np.array(
            rel_triplet[..., 1], dtype=np.int64)
        sub_boxes, obj_boxes = s_boxes.cpu().numpy(
        )[rel_s_ids], o_boxes.cpu().numpy()[rel_o_ids]
        sub_clses, obj_clses = s_clses.cpu().numpy(
        )[rel_s_ids], o_clses.cpu().numpy()[rel_o_ids]
        sub_scores, obj_scores = s_scores.cpu().numpy(
        )[rel_s_ids], o_scores.cpu().numpy()[rel_o_ids]
...
def build_model(cfg, device):
    ...
    weight_dict.update({'rel_loss_ce': cfg.LOSS.REL_CLS_COEF,
