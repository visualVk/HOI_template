...
class VisualModelV(nn.Module):
    def __init__(self, args, out_type=None):
        ...
        self.out_dim = 512
        self.appearence_in_dim = 2 * \
            (self.embedding_feature_dim + self.preprocess_dim)

        # pre process
        self.appearence_preprocess = nn.Linear(
            self.res_feat_dim, self.preprocess_dim)
        ...
        edge_dense_list = []
        for i in range(0, self.nr_frames):
            for j in range(0, self.nr_boxes):
                u, v = 0 + i * self.nr_boxes, j
                if v % self.nr_boxes == 0:
                    continue
                edge_dense_list.append((u, v))

        dense_src, dense_dst = tuple(zip(*edge_dense_list))
        self.spatial_dense_graph = dgl.graph((dense_src, dense_dst))
        self.spatial_dense_graph = dgl.to_bidirected(self.spatial_dense_graph)
        self.spatial_dense_graph = self.spatial_dense_graph.to('cuda')
        ...
        self.appearence_RNN = nn.RNN(
            input_size=self.appearence_in_dim,
            hidden_size=self.appearence_in_dim // 2,
            num_layers=1,
            batch_first=True,
            bidirectional=True)
        self.appearence_RNN.flatten_parameters()

        self.appearence_spatial_subnet = GAT(
            self.appearence_in_dim,
            -1,
            self.out_dim,
        ...
        self.spatial_temporal_subnet = GAT(
            self.appearence_in_dim,
            -1,
            self.out_dim,
            feat_drop=self.feat_drop,
            ...
            activation=nn.ReLU())
        # dense spatial graph
        self.spatial_dense_graph_subnet = GAT(
            self.appearence_in_dim,
            -1,
            self.appearence_in_dim,
            feat_drop=self.feat_drop,
        ...
        self.subact_frame_RNN = nn.RNN(
            input_size=2 * self.out_dim,
            hidden_size=2 * self.out_dim,
            num_layers=1,
            batch_first=True,
            bidirectional=True)
        self.afford_frame_RNN = nn.RNN(
            input_size=2 * self.out_dim,
            hidden_size=2 * self.out_dim,
            num_layers=1,
            batch_first=True,
    ...
    def forward(
        ...
        batch_spatial_graph = [
            self.spatial_graph for x in range(
                batch_size * self.nr_frames)]
        ...
        batch_dense_spatial_graph = [
            self.spatial_dense_graph for x in range(batch_size)]
        batch_dense_spatial_graph = dgl.batch(batch_dense_spatial_graph)

        # spatial
        box_input = box_input.transpose(2, 1).contiguous()
        box_input = box_input.view(
            batch_size * self.nr_boxes * self.nr_frames, 4)
        spatial_feats = self.coord_to_feature(box_input)

        # appearence
        appearence_feats = self.appearence_preprocess(node_features.reshape(
            batch_size * self.nr_boxes * self.nr_frames, self.res_feat_dim))

        # appearence_spatial
        appearence_spatial_feats = torch.cat(
            [spatial_feats, appearence_feats], dim=1)

        appearence_spatial_feats = appearence_spatial_feats.reshape(
            batch_size, self.nr_boxes, self.nr_frames, -1)
        appearence_spatial_node_feats = torch.zeros(
            (batch_size, self.nr_boxes, self.nr_frames, self.appearence_in_dim)).float().cuda()
        appearence_spatial_node_feats[:, 0, :, :self.appearence_in_dim //
                                      2] = appearence_spatial_feats[:, 0, :, :]
        appearence_spatial_node_feats[:, 1:, :, self.appearence_in_dim //
                                      2:] = appearence_spatial_feats[:, 1:, :, :]
        appearence_spatial_node_feats = self.spatial_dense_graph_subnet(
            batch_temporal_graph, appearence_spatial_node_feats.view(
                batch_size * self.nr_boxes * self.nr_frames, -1))

        appearence_spatial_node_feats = self.appearence_RNN(
            appearence_spatial_node_feats.reshape(
                batch_size *
                self.nr_boxes,
                self.nr_frames,
                self.appearence_in_dim))[0].reshape(
            batch_size,
            self.nr_boxes,
            self.nr_frames,
            self.appearence_in_dim).permute(
                0,
                2,
                1,
            3)
        ...
        appearence_spatial_subnet_node_feats = self.appearence_spatial_subnet(
            batch_spatial_graph, appearence_spatial_node_feats)
        appearence_spatial_subnet_node_feats = appearence_spatial_subnet_node_feats.reshape(
            batch_size, self.nr_frames, self.nr_boxes, self.out_dim)

        appearence_spatial_temporal_node_feats = self.spatial_temporal_subnet(
            batch_temporal_graph, appearence_spatial_node_feats)
        appearence_spatial_temporal_node_feats = appearence_spatial_temporal_node_feats.reshape(
            batch_size, self.nr_frames, self.nr_boxes, self.out_dim)
        ...
        subact_cls_scores = torch.sum(
            self.classifier_human(human_rnn_feats), dim=1)
        afford_cls_scores = torch.sum(
            self.classifier_object(obj_rnn_feats), dim=1)
...
class SemanticModelV(nn.Module):
    def __init__(self, args, out_type=None):
        ...
        self.category_embed_layer = nn.Embedding(
            12, self.embedding_feature_dim, padding_idx=0, scale_grad_by_freq=True)
        self.coord_to_feature = nn.Sequential(
        ...
        self.semantic_RNN = nn.RNN(
            input_size=self.semantic_in_dim,
            hidden_size=self.semantic_in_dim // 2,
            num_layers=1,
            batch_first=True,
            bidirectional=True)
        self.semantic_RNN.flatten_parameters()

        self.semantic_spatial_subnet = GAT(
    ...
    def forward(
        ...
        box_input = box_input.view(
            batch_size * self.nr_boxes * self.nr_frames, 4)
        spatial_feats = self.coord_to_feature(box_input)
        spatial_feats = spatial_feats.view(
            batch_size, self.nr_boxes, self.nr_frames, -1)
        ...
        box_categories = box_categories.view(
            batch_size * self.nr_boxes * self.nr_frames)
        box_category_embeddings = self.category_embed_layer(box_categories).view(
            batch_size, self.nr_boxes, self.nr_frames, self.embedding_feature_dim)

        # spatial_embedding
        spatial_embedding = torch.cat(
            [spatial_feats, box_category_embeddings], dim=3)

        semantic_spatial_node_feats = self.semantic_RNN(
            spatial_embedding.reshape(
                batch_size *
                self.nr_boxes,
                self.nr_frames,
                self.semantic_in_dim))[0].reshape(
            batch_size,
            self.nr_boxes,
            self.nr_frames,
            self.semantic_in_dim).permute(
                0,
                2,
                1,
            3)

        semantic_spatial_node_feats = semantic_spatial_node_feats.reshape(
            batch_size * self.nr_frames * self.nr_boxes, self.semantic_in_dim)

        semantic_spatial_subnet_node_feats = self.semantic_spatial_subnet(
            batch_spatial_graph, semantic_spatial_node_feats)
        semantic_spatial_subnet_node_feats = semantic_spatial_subnet_node_feats.reshape(
            batch_size, self.nr_frames, self.nr_boxes, self.out_dim)

        semantic_spatial_temporal_node_feats = self.spatial_temporal_subnet(
            batch_temporal_graph, semantic_spatial_node_feats)
        semantic_spatial_temporal_node_feats = semantic_spatial_temporal_node_feats.reshape(
            batch_size, self.nr_frames, self.nr_boxes, self.out_dim)

        spatial_temproal_feats = torch.cat(
            [semantic_spatial_subnet_node_feats, semantic_spatial_temporal_node_feats], dim=3)
