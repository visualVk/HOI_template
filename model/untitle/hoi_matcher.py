...
class HungarianPairMatcher(nn.Module):
    ...
    def reduce_redundant_gt_box(self, tgt_bbox, indices):
        ...
        """
        tgt_bbox_unique, map_idx, idx_cnt = torch.unique(
            tgt_bbox, dim=0, return_inverse=True, return_counts=True)
    ...
    def forward(self, outputs, targets, indices, log=False):
        ...
        detr_query_num = outputs["pred_logits"].shape[1] if (
            outputs["pred_oidx"].shape[-1] == (outputs["pred_logits"].shape[1] + 1)) else -1
        ...
        for batch_idx in range(bs):
            ...
            if self.is_vcoco:
                ...
                keep_idx = (
                    targets[batch_idx]["pair_actions"].sum(
                        dim=-1) != 0)
                ...
                # (num_pair_boxes, 8)
                tgt_pbox = targets[batch_idx]["pair_boxes"]
                # (num_pair_boxes, 29)
                tgt_act = targets[batch_idx]["pair_actions"]
                # (num_pair_boxes)
                tgt_tgt = targets[batch_idx]["pair_targets"]
                ...
                # (num_pair_boxes, 4)
                tgt_hbox = targets[batch_idx]["sub_boxes"]
                # (num_pair_boxes, 4)
                tgt_obox = targets[batch_idx]["obj_boxes"]

            # find which gt boxes match the h, o boxes in the pair
            if self.is_vcoco:
                hbox_with_cls = torch.cat([tgt_hbox, torch.ones(
                    (tgt_hbox.shape[0], 1)).to(tgt_hbox.device)], dim=1)
            elif self.is_hico:
                hbox_with_cls = torch.cat([tgt_hbox, torch.zeros(
                    (tgt_hbox.shape[0], 1)).to(tgt_hbox.device)], dim=1)
            obox_with_cls = torch.cat([tgt_obox, tgt_tgt.unsqueeze(-1)], dim=1)
            # turn the class of occluded objects to -1
            obox_with_cls[obox_with_cls[:, :4].sum(dim=1) == -4, -1] = -1

            bbox_with_cls = torch.cat([tgt_bbox, tgt_cls.unsqueeze(-1)], dim=1)
            bbox_with_cls, k_idx, bbox_idx = self.reduce_redundant_gt_box(
                bbox_with_cls, indices[batch_idx])
            bbox_with_cls = torch.cat((bbox_with_cls, torch.as_tensor(
                [-1.] * 5).unsqueeze(0).to(tgt_cls.device)), dim=0)
            ...
            h_match_indices = torch.nonzero(
                cost_hbox == 0, as_tuple=False)  # (num_hbox, num_boxes)
            o_match_indices = torch.nonzero(
                cost_obox == 0, as_tuple=False)  # (num_obox, num_boxes)
            ...
            if tgt_act.shape[0] == 0:
                tgt_act = torch.zeros(
                    (1, tgt_act.shape[1])).to(
                    targets[batch_idx]["pair_actions"].device)
                targets[batch_idx]["pair_actions"] = torch.zeros(
                    (1, targets[batch_idx]["pair_actions"].shape[1])).to(
                    targets[batch_idx]["pair_actions"].device)
                if self.is_hico:
                    pad_tgt = -1  # outputs["pred_obj_logits"].shape[-1]-1
                    tgt_tgt = torch.tensor([pad_tgt]).to(
                        targets[batch_idx]["pair_targets"])
                    targets[batch_idx]["pair_targets"] = torch.tensor(
                        [pad_tgt]).to(targets[batch_idx]["pair_targets"].device)
            ...
            if self.is_vcoco:
                out_act[..., self.invalid_ids] = 0
            if self.is_hico:
                out_tgt = outputs["pred_obj_logits"][batch_idx].softmax(-1)
                out_tgt[..., -1] = 0  # don't get cost for no-object
            tgt_act = torch.cat([tgt_act, torch.zeros(
                tgt_act.shape[0]).unsqueeze(-1).to(tgt_act.device)], dim=-1)

            # [batch_size * num_queries, detr.num_queries+1]
            cost_hclass = -out_hprob[:, tgt_hids]
            # [batch_size * num_queries, detr.num_queries+1]
            cost_oclass = -out_oprob[:, tgt_oids]

            cost_pos_act = (-torch.matmul(out_act,
                            tgt_act.t().float())) / tgt_sum
            cost_neg_act = (torch.matmul(out_act, (~tgt_act.bool()).type(torch.int64).t(
            ).float())) / (~tgt_act.bool()).type(torch.int64).sum(dim=-1).unsqueeze(0)
            ...
            log_act_cost = torch.zeros(
                [1]).to(
                tgt_act.device) if tgt_act.shape[0] == 0 else act_cost.min(
                dim=0)[0].mean()
        ...
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(
            j, dtype=torch.int64)) for i, j in return_list], targets
