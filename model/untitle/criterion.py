...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, num_actions=None, HOI_losses=None,
                 HOI_matcher=None, args=None):
    ...
    def loss_pair_actions(self, outputs, targets, hoi_indices, num_boxes):
        ...
        pos_classes = torch.full(target_classes[idx].shape, 0, dtype=torch.float32,
                                 device=src_actions.device)  # else, the last index for no-interaction is '0'
        ...
        loss_bce = F.binary_cross_entropy(logits[..., self.valid_ids], target_classes[..., self.valid_ids],
                                          reduction='none')
        p_t = logits[..., self.valid_ids] * target_classes[..., self.valid_ids] + (1 - logits[..., self.valid_ids]) * (
                    1 - target_classes[..., self.valid_ids])
    ...
    def loss_pair_targets(self, outputs, targets, hoi_indices, num_interactions, log=True):
        ...
        loss_obj_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.tgt_empty_weight,
                                      ignore_index=-1)
        losses = {'loss_tgt': loss_obj_ce}

        if log:
            ignore_idx = (target_classes_o != -1)
            losses['obj_class_error'] = 100 - accuracy(src_logits[idx][ignore_idx, :-1], target_classes_o[ignore_idx])[
                0]
