    ...
    def forward(self, results, outputs, orig_target_sizes, max_target_sizes):
        ...
        for i, (cur_mask, t, tt) in enumerate(
            ...
            results[i]["masks"] = F.interpolate(
                results[i]["masks"].float(), size=tuple(
                    tt.tolist()), mode="nearest").byte()
...
class PostProcessPanoptic(nn.Module):
    ...
    def forward(self, outputs, processed_sizes, target_sizes=None):
        ...
        for cur_logits, cur_masks, cur_boxes, size, target_size in zip(
            ...
            # In the following, we track the list of masks ids for each stuff
            # class (they are merged later on)
            ...
            def get_ids_area(masks, scores, dedup=False):
                ...
                # It also returns the area of the masks that appears on the
                # image
            ...
            if cur_classes.numel() > 0:
                ...
                while True:
                    filtered_small = torch.as_tensor([area[i] <= 4 for i, c in enumerate(
                        cur_classes)], dtype=torch.bool, device=keep.device)
