...
class ModifiedEncoderLayer(nn.Module):
    def __init__(
        ...
        self.attn = nn.ModuleList(
            [nn.Linear(3 * self.sub_repr_size, 1) for _ in range(num_heads)])
...
class InteractionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.comp_layer = transformer.TransformerEncoderLayer(
    ...
    def forward(
        ...
        for b_idx, props in enumerate(region_props):
            ...
            if n_h == 0 or n <= 1:
                ...
                boxes_h_collated.append(
                    torch.zeros(
                        0,
                        device=device,
                        dtype=torch.int64))
                boxes_o_collated.append(
                    torch.zeros(
                        0,
                        device=device,
                        dtype=torch.int64))
                object_class_collated.append(torch.zeros(
            ...
            unary_tokens, unary_attn = self.coop_layer(
                unary_tokens, box_pair_spatial_reshaped)
