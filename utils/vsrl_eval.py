...
class VCOCOeval(object):
        ...
        if 0:
            ...
            for entry in vcocodb:
                for aid in range(self.num_actions):
                    nums[aid] += np.sum(np.logical_and(entry['gt_actions']
                                        [:, aid] == 1, entry['gt_classes'] == 1))
            for aid in range(self.num_actions):
    ...
    def _prep_vcocodb_entry(self, entry):
        ...
        entry['gt_role_id'] = np.empty(
            (0, self.num_actions, 2), dtype=np.int32)
    ...
    def _add_gt_annotations(self, entry):
        ...
        gt_actions = -np.ones((num_valid_objs, self.num_actions),
                              dtype=entry['gt_actions'].dtype)
        gt_role_id = - \
            np.ones((num_valid_objs, self.num_actions, 2), dtype=entry['gt_role_id'].dtype)
        ...
        entry['gt_actions'] = np.append(
            entry['gt_actions'], gt_actions, axis=0)
        entry['gt_role_id'] = np.append(
            entry['gt_role_id'], gt_role_id, axis=0)
    ...
    def _collect_detections_for_image(self, dets, image_id):
        ...
        for det in dets:
            if det['image_id'] == image_id:
                this_agent = np.zeros(
                    (1, 4 + self.num_actions), dtype=np.float32)
                this_role = np.zeros(
                    (1, 5 * self.num_actions, 2), dtype=np.float32)
                this_agent[0, :4] = det['person_box']
                for aid in range(self.num_actions):
                    for j, rid in enumerate(self.roles[aid]):
                        if rid == 'agent':
                            this_agent[0,
                                       4 + aid] = det[self.actions[aid] + '_' + rid]
                        else:
                            this_role[0, 5 * aid: 5 * aid + 5, j -
                                      1] = det[self.actions[aid] + '_' + rid]
    ...
    def _do_eval(self, detections_file, ovr_thresh=0.5):
        ...
        mAP_a = self._do_agent_eval(
            vcocodb, detections_file, ovr_thresh=ovr_thresh)
        mAP_r_1 = self._do_role_eval(
            vcocodb,
            detections_file,
            ovr_thresh=ovr_thresh,
            eval_type='scenario_1')
        mAP_r_2 = self._do_role_eval(
            vcocodb,
            detections_file,
            ovr_thresh=ovr_thresh,
            eval_type='scenario_2')
        return mAP_a, mAP_r_1, mAP_r_2
    ...
    def _do_role_eval(
        ...
        for i in range(len(vcocodb)):
            ...
            assert np.all(gt_actions[np.where(ignore)[0]] == -1)
            ...
            for aid in range(self.num_actions):
                ...
                for rid in range(len(self.roles[aid]) - 1):
                    ...
                    # keep track of detected instances for each action for each
                    # role
                        ...
                        if np.all(gt_roles[jmax, :] == -1):  # if no gt role
                            if eval_type == 'scenario_1':
                                if np.all(role_boxes[j, :] == 0.0) or np.all(
                                    ...
                                    # if no role is predicted, mark it as
                                    # correct role overlap
                                    ov_role = 1.0
                                else:
                                    # if a role is predicted, mark it as false
                                    ov_role = 0.0
                            elif eval_type == 'scenario_2':
                                # if no gt role, role prediction is always
                                # correct, irrespective of the actual predition
                                ov_role = 1.0
                            else:
                                raise ValueError('Unknown eval type')
                        else:
                            ov_role = get_overlap(
                                gt_roles[jmax, :].reshape((1, 4)), role_boxes[j, :])

                        is_true_action = (gt_actions[jmax, aid] == 1)
                        sc[aid][rid].append(agent_scores[j])
                        if is_true_action and (
                                ovmax >= ovr_thresh) and (
                                ov_role >= ovr_thresh):
                            if covered[jmax]:
                                fp[aid][rid].append(1)
                                tp[aid][rid].append(0)
        ...
        for aid in range(self.num_actions):
            ...
            for rid in range(len(self.roles[aid]) - 1):
                print('{: >23}: AP = {:0.2f} (#pos = {:d})'.format(
                    self.actions[aid] + '-' + self.roles[aid][rid + 1], role_ap[aid, rid] * 100.0, int(npos[aid])))
        print('Average Role [%s] AP = %.2f' %
              (eval_type, np.nanmean(role_ap) * 100.00))
        print('---------------------------------------------')
        return eval_type, np.nanmean(role_ap) * 100.00
    ...
    def _do_agent_eval(self, vcocodb, detections_file, ovr_thresh=0.5):
        ...
        for aid in range(self.num_actions):
            print('{: >20}: AP = {:0.2f} (#pos = {:d})'.format(
                self.actions[aid], agent_ap[aid] * 100.0, int(npos[aid])))
        print('Average Agent AP = %.2f' %
              (np.nansum(agent_ap) * 100.00 / self.num_actions))
        print('---------------------------------------------')
...
def _load_vcoco(vcoco_file):
    ...
    for i in range(len(vsrl_data)):
        vsrl_data[i]['role_object_id'] = np.array(
            vsrl_data[i]['role_object_id']).reshape(
            (len(
                vsrl_data[i]['role_name']), -1)).T
